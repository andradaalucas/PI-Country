...
function ActivityCreate() {
  ...
  function handleSubmit(e) {
    ...
    alert(
      `Añadiste la actividad ${activity.name} en ${activity.nameCountry}🤩`
    );
  }

  function validar(data) {
    let errores = {};
    if (validarName(data.name))
      errores.name = "El nombre debe ser mayor a 3 letras";
    if (!data.difficulty || typeof data.difficulty !== "string")
      errores.difficulty = "* El campo dificultad es obligatorio";
    if (!data.season || typeof data.season !== "string")
      errores.season = "* El campo temporada es obligatorio";
    if (!data.duration || typeof data.duration !== "string")
      errores.duration = "* El campo duracion es obligatorio";
    if (
      !data.nameCountry.length ||
      data.nameCountry === "seleccionaPais" ||
      data.nameCountry === ""
    )
      errores.nameCountry = "* El campo Pais es obligatorio";
    return errores;
  }
    ...
    <div>
      ...
      <form className="form" onSubmit={handleSubmit}>
        <div className="container">
          ...
          {errorForm.name && errorForm.season ? (
            <p>
              <small>{errorForm.season}</small>
            </p>
          ) : (
            false
          )}
          ...
          {errorForm.season && errorForm.duration ? (
            <p>
              <small>{errorForm.duration}</small>
            </p>
          ) : (
            false
          )}
          ...
          <input
            ...
            name="duration"
          />
          {errorForm.season && errorForm.difficulty ? (
            <p>
              <small>{errorForm.difficulty}</small>
            </p>
          ) : (
            false
          )}
          ...
          {errorForm.nameCountry ? (
            <p>
              <small>{errorForm.nameCountry}</small>
            </p>
          ) : (
            false
          )}
